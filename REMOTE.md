...
2. Under **Manage Local Backend**, toggle **Remote access** on to use the recommended Tailnet mode. The same row also keeps a **Use LAN instead** action available from the local-only state, so you can intentionally choose LAN mode without first enabling Tailnet.
...
5. Tailnet remains the recommended desktop path. If Tailscale detection fails or you need a generic private-network or LAN address instead, use the **Use LAN instead** action in the same settings panel to switch modes.
...
For the printed headless pairing host, `t3 serve` uses this precedence:

1. The explicit bind host from `--host` or `T3CODE_HOST`
2. `T3CODE_TAILNET_HOST`, if set
3. `TS_CERT_DOMAIN`, if set
4. The first result from `tailscale ip -4`
5. Fallback behavior: wildcard binds fall back to a LAN address, and no explicit host falls back to `localhost`

Examples:

```bash
npx t3 serve
T3CODE_TAILNET_HOST=mackbook.tailnet.ts.net npx t3 serve
npx t3 serve --host 100.88.12.4
```
