...
export default function BrowserPanel({
  ...
  const navRowClassName = cn(
    "flex h-11 min-h-11 items-center gap-1.5 border-b border-border/80 bg-card/94 px-2.5",
    isElectron && "[-webkit-app-region:no-drag]",
  );
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-card text-foreground">
      ...
      >
        ...
        {cookieManager ? (
          <Sheet open={cookieManagerOpen} onOpenChange={setCookieManagerOpen}>
            <SheetTrigger
              render={
                <span className="inline-flex">
                  <Button
            ...
            <SheetPopup side="right" variant="inset">
              <SheetPanel className="pt-6">{cookieManager}</SheetPanel>
            </SheetPopup>
          </Sheet>
        ) : null}
        <ToolbarIconButton
