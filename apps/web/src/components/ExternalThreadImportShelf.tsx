import type { ExternalThreadImportDiscoveryResult, ThreadId } from "@t3tools/contracts";
import {
  ChevronDownIcon,
  CircleAlertIcon,
  DownloadIcon,
...
import { providerDisplayName } from "./ExternalThreadImportDialog.logic";
import {
...
export function ExternalThreadImportShelf({
  ...
  const load = useCallback(async () => {
    ...
    const errors = targetResults.flatMap((result) => ("error" in result ? [result.error] : []));

    if (generation !== generationRef.current) return;
    setError(errors.length > 0 ? failureMessage(errors[0]) : null);
    setLoading(false);
  }, [discover, targets]);
