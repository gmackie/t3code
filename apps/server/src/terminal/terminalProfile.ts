...
import { type ShellCandidate } from "./Manager.ts";
...
export async function discoverWindowsTerminalShells(input: {
  ...
  const cmdPath = await firstExistingPath(
    [input.env.ComSpec ?? NodePath.win32.join(systemRoot, "System32", "cmd.exe")],
    input.probe,
  );
  ...
  const wslPath = await firstExistingPath(
    [NodePath.win32.join(systemRoot, "System32", "wsl.exe")],
    input.probe,
  );
