...
import type { SmolAgentAdapterShape } from "../Services/SmolAgentAdapter.ts";
...
const fakeSmolAgentAdapter: SmolAgentAdapterShape = {
  provider: "smolAgent",
  ...
  Layer.mergeAll(
    Layer.provide(
      ...
      Layer.mergeAll(
        ...
        Layer.succeed(SmolAgentAdapter, fakeSmolAgentAdapter),
      ),
    ),
    NodeServices.layer,
  ),
...
layer("ProviderAdapterRegistryLive", (it) => {
  it.effect("resolves a registered provider adapter", () =>
    Effect.gen(function* () {
      ...
      const smolAgent = yield* registry.getByProvider("smolAgent");
      assert.equal(codex, fakeCodexAdapter);
      assert.equal(claude, fakeClaudeAdapter);
      assert.equal(openCode, fakeOpenCodeAdapter);
      assert.equal(cursor, fakeCursorAdapter);
      assert.equal(smolAgent, fakeSmolAgentAdapter);

      const providers = yield* registry.listProviders();
      assert.deepEqual(providers, ["codex", "claudeAgent", "opencode", "cursor", "smolAgent"]);
    }),
  );
