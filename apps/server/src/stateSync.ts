...
function shouldSkipRelativePath(
  ...
  const [topLevelName] = relativePath.split(NodePath.sep);
...
async function collectSourceFiles(input: {
  ...
  for (const entry of await NodeFSP.readdir(input.currentDir, { withFileTypes: true })) {
    const absolutePath = NodePath.join(input.currentDir, entry.name);
    const relativePath = NodePath.relative(input.rootDir, absolutePath);
...
export function resolveT3StateProfile(
  ...
  const baseDir = NodePath.join(options?.homeDir ?? NodeOS.homedir(), defaults.baseDirName);
  return {
    baseDir,
    stateDir: NodePath.join(baseDir, defaults.stateDirName),
  };
}
...
async function removeTargetSqliteStateFiles(targetStateDir: string): Promise<void> {
  await Promise.all(
    SQLITE_STATE_FILE_NAMES.map((fileName) =>
      NodeFSP.rm(NodePath.join(targetStateDir, fileName), { force: true }),
    ),
  );
}
...
export async function syncT3State(input: {
  ...
  const sourceStat = await NodeFSP.stat(input.sourceStateDir);
  ...
  await NodeFSP.mkdir(input.targetStateDir, { recursive: true });
  await removeTargetSqliteStateFiles(input.targetStateDir);
  await NodeFSP.cp(input.sourceStateDir, input.targetStateDir, {
    recursive: true,
    force: true,
    filter: (sourcePath) => {
      const relativePath = NodePath.relative(input.sourceStateDir, sourcePath);
      if (relativePath === "") {
        return true;
      }
