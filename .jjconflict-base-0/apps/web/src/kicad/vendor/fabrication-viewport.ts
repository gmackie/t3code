 ...
 */
export function paneLayout(drawn: BoardRect, board: BoardRect, camera: Camera, pane: PaneSize) {
...
export function useBoardViewport(board: BoardRect | null) {
  ...
  const home = useMemo(() => (board ? centreCamera(board) : { scale: 1, cx: 0, cy: 0 }), [board]);
